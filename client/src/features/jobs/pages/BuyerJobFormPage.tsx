...
import { createJob, importJobs, updateJob, type CreateJobRequest } from '../../../api/jobsApi';
...
export function BuyerJobFormPage() {
  ...
  const isSupplierManualMode = location.pathname.startsWith('/supplier/jobs/new');
  ...
  useEffect(() => {
    if (isSupplierManualMode || isEditMode || buyerName || !session?.company?.name) return;

    setBuyerName(session.company.name);
  }, [buyerName, isEditMode, isSupplierManualMode, session?.company?.name]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isSupplierManualMode) {
        const imported = await importJobs([{ ...payload, sourceType: 'manual', status: 'open' }]);
        const savedJob = imported.items[0];
        navigate(savedJob ? getJobDetailPath('supplier', savedJob.id) : '/supplier/jobs', { replace: true });
        return;
      }
      ...
      setErrorMessage(getApiErrorMessage(error, isSupplierManualMode ? '입찰공고 수동 입력 중 오류가 발생했습니다.' : isEditMode ? '공고 수정 중 오류가 발생했습니다.' : '공고 등록 중 오류가 발생했습니다.'));
    } finally {
  ...
  return (
    <section>
      <PageTitle
        title={isSupplierManualMode ? '입찰공고 수동 입력' : isEditMode ? '공고 수정' : '신규 공고 등록'}
        description={isSupplierManualMode ? '외부에서 확인한 입찰공고를 공급기관 검토 목록에 직접 등록합니다.' : '발주 관점에서 검토하거나 관리할 사업 공고와 제안 접수, 평가, 계약 전환에 필요한 기준 정보를 등록합니다.'}
        actions={
          <>
            ...
            <Button type="submit" form="job-form" icon={<Save className="h-4 w-4" />} disabled={isSubmitting}>
              {isSubmitting ? '저장 중...' : isSupplierManualMode ? '수동 입력 저장' : isEditMode ? '수정 저장' : '공고 등록'}
            </Button>
          </>
        }
      />
        ...
        <div className="space-y-8">
          <Card className="p-8">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <Input label="발주기관" placeholder="예: 조달청, 교육부, 한국도로공사" value={buyerName} onChange={(event) => setBuyerName(event.target.value)} required disabled={!isSupplierManualMode && !isEditMode} />
